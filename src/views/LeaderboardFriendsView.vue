...
onMounted(async () => {
  ...
  try {
    ...
    const allfriendsResponse = await friendingStore._getFriends(authStore.userId || '');
    if ('error' in allfriendsResponse) {
        console.error("Error getting friends:", allfriendsResponse.error);
        return;
    }   else {
      allusers.value = allfriendsResponse.map(friend => friend.friend);
      allusers.value.push(authStore.userId || ''); // include self
    }
    console.log("All users fetched2:", allusers);
  } catch (error) {
...
<template>
  <div class="leaderboard-view">
    <Leaderboard :items="players" title="Friends Top Scores" />
  </div>
</template>
